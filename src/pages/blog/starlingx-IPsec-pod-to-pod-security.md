...
author: Andy Ning
date: 2025-10-24T01:32:05.627Z
category:
  - label: Features & Updates
    id: category-A7fnZYrE1
---
# Overview
In stx 11.0, a new feature called IPsec pod-to-pod security has been introduced. With this feature, inter-host pod-to-pod traffic for a service can be selectively configured to be protected by IPsec in tunnel mode over the cluster host network. 
...
You need to install the ipsec-policy-operator system application first in order to protect the inter-host pod-to-pod traffic for services. Once installed, you can then define IPsec policies in a yaml file and apply the yaml file using "kubectl" command to create IPsec policies for the services.

You can update the existing IPsec polices by updating and re-applying the yaml file, or using "kubectl edit" command to update existing policy CRs directly.
...
Notes:
  ...
  - The IPsec policies can be created before or after a service is deployed. The policies can be created as part of the service deployment.
...
The ipsec-policy-operator system application can be removed, if required, by running the following command:
